...
const BarRow = ({ label, color, pct, value, theme }: { label: string; color: string; pct: number; value: number; theme: any }) => (
  <Stack $gap="4px">
    ...
    <Box $bg="transparent" $radius="pill" style={{ width: "100%", height: 8, overflow: "hidden", border: `1px solid ${theme.colors.neutral.greyBlue}` }}>
      ...
      <Stack $gap="md">
        ...
        {hasBreakdown && totalQuestions > 0 ? (
          <Stack $gap="md">
            ...
            {stats?.total_closed_questions !== undefined && (
              <BarRow label="Zamknięte" color="rgba(33, 150, 243, 0.65)" pct={pct(stats.total_closed_questions)} value={stats.total_closed_questions} theme={theme} />
            )}
            {stats?.total_open_questions !== undefined && (
              <BarRow label="Otwarte" color="rgba(156, 39, 176, 0.65)" pct={pct(stats.total_open_questions)} value={stats.total_open_questions} theme={theme} />
            )}
            {stats?.total_easy_questions !== undefined && (
              <BarRow label="Łatwe" color="rgba(76, 175, 80, 0.8)" pct={pct(stats.total_easy_questions)} value={stats.total_easy_questions} theme={theme} />
            )}
            {stats?.total_medium_questions !== undefined && (
              <BarRow label="Średnie" color="rgba(255, 193, 7, 0.9)" pct={pct(stats.total_medium_questions)} value={stats.total_medium_questions} theme={theme} />
            )}
            {stats?.total_hard_questions !== undefined && (
              <BarRow label="Trudne" color="rgba(244, 67, 54, 0.9)" pct={pct(stats.total_hard_questions)} value={stats.total_hard_questions} theme={theme} />
            )}
          </Stack>
        ) : (
          <Text $variant="body3" $tone="muted">
